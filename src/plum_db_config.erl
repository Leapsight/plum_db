...
%%  plum_db_config.erl -
%%
%%  Copyright (c) 2016-2018 Leapsight t/a Leapsight. All rights reserved.
%%
...
coerce_partitions() ->
    ...
    case length(Subdirs) of
        ...
        M ->
            ...
            _ = lager:warning(
                "The number of partitions on disk differ from the configuration. In order to protect the existing data the requested value will be ignored and the configuration will be coerced to the existing value; requested=~p, existing=~p, data_dir=~p",
                [N, M, DataDir]
            ),
            set(partitions, M)
    end.
